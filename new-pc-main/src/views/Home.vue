      ...
      <aside class="left-col">
        ...
        <section class="panel compact-panel">
          ...
          <div class="mini-item">
            ...
            <span>大厅区：可交流、可充电、剩余座位 {{ loungeRemain }} 个</span>
          </div>
...
import { createReservationApi, getMyReservationsApi } from '../api/reservation'
...
const floors = [...MAP_FLOORS, '服务台']
const bookableFloors = [...MAP_FLOORS]
...
const currentUser = computed(() => ({
  ...
  studentNo: userStore.userInfo?.studentNo || userStore.userInfo?.student_no || ''
}))
...
async function loadSeatsFromBackend() {
  ...
  try {
    seatsByFloor.value = createLegacySeatMapByFloor()
...
const loungeRemain = computed(() => {
  ...
  return allSeats.filter((seat) => seat.area === '大厅区' && seat.status === 'available').length
})
...
async function confirmReservationFinal() {
  ...
  try {
    const seatType = selectedSeat.value.window ? 'window' : selectedSeat.value.power ? 'power' : selectedSeat.value.silent ? 'silent' : 'normal'
    const res = await createReservationApi({
      seatId: selectedSeat.value.id,
      seatNo: pendingReservation.value.seatNo,
      seatType,
      startTime: pendingReservation.value.startISO,
      endTime: pendingReservation.value.endISO
    })

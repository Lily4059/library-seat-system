        ...
        >
          ...
          <div class="card-actions">
            ...
            <template v-else-if="item.status === 'checked_in'">
              <el-button round type="primary" @click="checkOut(item)">签退</el-button>
            </template>
...
import { getMyReservationsApi, cancelReservationApi, checkInApi, checkOutApi } from '../api/reservation'
...
const checkIn = async (reservation) => {
  try {
    ...
    const res = await checkInApi(reservation.id)
    if (res?.data?.code === 200) {
...
const checkOut = async (reservation) => {
  try {
    await ElMessageBox.confirm(`确认签退座位 ${reservation.seatNo} 吗？`, '签退确认', {
      confirmButtonText: '确认',
      cancelButtonText: '取消',
      type: 'info'
    })
    const res = await checkOutApi(reservation.id)
    if (res?.data?.code === 200) {
      const target = reservations.value.find((r) => r.id === reservation.id)
      if (target) {
        target.status = 'completed'
        target.checkOutTime = res.data?.data?.checkOutTime
      }
      const studyTime = res.data?.data?.studyTime || 0
      const hours = Math.floor(studyTime / 60)
      const mins = studyTime % 60
      const timeStr = hours > 0 ? `${hours}小时${mins}分钟` : `${mins}分钟`
      ElMessage.success(`签退成功，本次学习时长：${timeStr}`)
    } else {
      ElMessage.error(res?.data?.message || '签退失败')
    }
  } catch (error) {
    if (error !== 'cancel') ElMessage.error('签退失败')
  }
}
...
const cancelReservation = async (reservation) => {
  try {
    ...
    const res = await cancelReservationApi(reservation.id)
    if (res?.data?.code === 200) {
      const target = reservations.value.find((r) => r.id === reservation.id)

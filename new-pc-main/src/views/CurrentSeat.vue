...
import { getMyReservationsApi, checkInApi, cancelReservationApi } from '../api/reservation'
...
const handleCheckIn = async () => {
  ...
  try {
    const res = await checkInApi(currentReservation.value.id)
    if (res.data.code === 200) {
...
const handleCancel = async () => {
  ...
  actionLoading.value = true
  try {
    const res = await cancelReservationApi(currentReservation.value.id)
    if (res.data.code === 200) {
